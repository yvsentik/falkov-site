...
export const site = {
	...
	// Решение владельца 14.09.2026: заявки снова уходят напрямую в бота, спам через открытый токен
	// принимаем как издержку. Перед формой стоит капча Яндекса, она включается вместе с этим токеном.
	// Заявки уходят напрямую в бота. Решение владельца: токен лежит в клиенте, спам принимаем как издержку.
	// НЕ убирать токен и не переводить форму в режим «открыть чат»: так уже теряли заявки 14–21.09.2026.
	telegramBot: { token: '8656918564:AAHZQW_q74De5ax4Vwp1b3fN3Jhk4nMRLjI', chatId: '7315203539' }, // @falkov_leads_bot → Роман, токен от 14.09.2026
	...
	formEndpoint: '',
	// Дубль заявки письмом на почту владельца: PHP-скрипт на хостинге reg.ru (hosting/lead.php).
	// Работает параллельно с ботом и его не заменяет; если скрипт недоступен, заявка всё равно уйдёт в бота.
	mailEndpoint: 'https://api.falkov-marketing.ru/lead.php',
