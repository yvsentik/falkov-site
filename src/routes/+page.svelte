<script>
	...
	function topQueries(c, limit = 5) {
		...
		/* Показываем только то, что стоит в ТОП-5: точные позиции не публикуем –
		   выдача шевелится. «ТОП-5» проверяется руками в любой день. */
...
<div class="fk">
	...
	<section class="fk-sec" id="keysy">
		<div class="fk-sec__head">
			<h2>Кейсы</h2>
			<p class="fk-note">
				Скриншоты позиций и метрики – смотрите цифры сами. Часть клиентов под NDA, поэтому где-то ниша и
				регион закрыты.
			</p>
		</div>
	...
	<section class="fk-sec fk-sec--tight" id="agentstvo">
		<div class="fk-about">
			<div class="fk-about__text" use:reveal>
				...
				<h2>Работаем лично: встречаемся и показываем</h2>
				<p>
...
<style>
	...
	/* скриншот сайта показываем «шапкой»: берём верх страницы без растяжки */
	.fk-shot--site {
		aspect-ratio: 16 / 10;
		background: #fff;
	}
	...
	.fk-marquee img {
		/* высота в пикселях: в grid-ячейке процент
